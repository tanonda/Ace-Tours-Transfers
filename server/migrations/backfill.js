...
var tours = pgTable("tours", {
  ...
  // DEPRECATED: use adultPriceCents
  childPrice: text("child_price"),
  // DEPRECATED: use childPriceCents
  ...
  // { make, model, seats, transmission, features[] }
});
var tourInstances = pgTable("tour_instances", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  ...
  instanceUniqueIdx: index("idx_tour_instances_unique").on(table.tourId, table.serviceDate, table.timeSlot)
}));
var availabilityHolds = pgTable("availability_holds", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
...
var bookings = pgTable("bookings", {
  ...
  tourId: varchar("tour_id").notNull().references(() => tours.id),
  // Legacy: First item for quick ref
  ...
  // DEPRECATED: use totalAmountCents
...
var contentBlocks = pgTable("content_blocks", {
  ...
  // e.g., 'hero', 'featured-tours', 'promotions'
...
var siteSettings = pgTable("site_settings", {
  ...
  // e.g., 'whatsapp_number', 'whatsapp_greeting'
...
var paymentGateways = pgTable("payment_gateways", {
  ...
  // NEW: Priority for failover logic
...
var cmsContent = pgTable("cms_content", {
  ...
  // references content_blocks slug
  contentKey: text("content_key").notNull(),
  // e.g., 'heading', 'description', 'image'
  contentType: text("content_type").notNull().default("text"),
  // 'text', 'image', 'richtext'
  value: text("value"),
  // The actual content or image URL
  locale: text("locale").default("en"),
  // For i18n support
...
var payments = pgTable("payments", {
  ...
  // Amount in smallest currency unit (e.g., cents/vatu)
  ...
  reconciliationStaleIdx: index("idx_payments_reconciliation_stale").on(table.status, table.lastReconciledAt, table.createdAt).where(sql`status = 'processing'`),
  pendingExpiryIdx: index("idx_payments_pending_expiry").on(table.status, table.expiresAt).where(sql`status IN ('pending', 'processing')`)
}));
var bookingSummaries = pgTable("booking_summaries", {
...
var revenueDaily = pgTable("revenue_daily", {
...
var paymentOverviews = pgTable("payment_overviews", {
...
var toursRelations = relations(tours, ({ many }) => ({
  bookings: many(bookings)
}));
...
var bookingItemsRelations = relations(bookingItems, ({ one }) => ({
  booking: one(bookings, {
    fields: [bookingItems.bookingId],
    references: [bookings.id]
  })
}));
var tourInstancesRelations = relations(tourInstances, ({ one, many }) => ({
  tour: one(tours, {
...
var availabilityHoldsRelations = relations(availabilityHolds, ({ one }) => ({
  tourInstance: one(tourInstances, {
    fields: [availabilityHolds.tourInstanceId],
    references: [tourInstances.id]
  }),
  booking: one(bookings)
  // This might need a field if it's 1:1, but many bookings could technically exist for a hold if we failed something? Usually 1:1.
}));
...
var paymentGatewaysRelations = relations(paymentGateways, ({ many }) => ({
  payments: many(payments)
}));
...
var cmsContentRelations = relations(cmsContent, ({ one }) => ({
...
var insertBookingItemSchema = createInsertSchema(bookingItems).omit({
  id: true,
  createdAt: true
});
...
var MastercardGatewayCredentialsSchema = external_exports.object({
  merchantId: external_exports.string(),
  accessCode: external_exports.string(),
  secureHashSecret: external_exports.string(),
  apiEndpoint: external_exports.string().url(),
  version: external_exports.string().optional()
  // API Version if required
});
var StripeCredentialsSchema = external_exports.object({
  secretKey: external_exports.string(),
  publishableKey: external_exports.string().optional(),
  webhookSecret: external_exports.string()
});
var GooglePayCredentialsSchema = external_exports.object({
  merchantId: external_exports.string(),
  gateway: external_exports.string(),
  // e.g., "paypal" or "bred"
  gatewayMerchantId: external_exports.string().optional()
});
var ApplePayCredentialsSchema = external_exports.object({
  merchantIdentifier: external_exports.string(),
  domainName: external_exports.string().url(),
  paymentProcessingCertificateUrl: external_exports.string().url().optional(),
  // URL to uploaded cert
  gateway: external_exports.string()
  // e.g., "paypal" or "bred"
});
var PayPalCredentialsSchema = external_exports.object({
  clientId: external_exports.string(),
  clientSecret: external_exports.string(),
  mode: external_exports.enum(["sandbox", "live"]),
  merchantAccountType: external_exports.enum(["BUSINESS_VERIFIED", "PERSONAL_UNVERIFIED"]).optional(),
  ipnWebhookUrl: external_exports.string().url().optional(),
  settlementCurrency: external_exports.string().optional(),
  checkoutExperience: external_exports.enum(["PAY_WITH_PAYPAL", "PAY_WITH_CARD_OR_PAYPAL"]).optional()
});
var EWalletCredentialsSchema = external_exports.object({
  apiKey: external_exports.string(),
  apiSecret: external_exports.string()
}).partial();
var GenericLocalBankCredentialsSchema = external_exports.object({
  bankName: external_exports.string(),
  accountName: external_exports.string(),
  accountNumber: external_exports.string(),
  swiftCode: external_exports.string().optional()
}).partial();
var WanTokCredentialsSchema = external_exports.object({
  merchantId: external_exports.string(),
  apiKey: external_exports.string(),
  apiSecret: external_exports.string(),
  integrationType: external_exports.enum(["API", "USSD", "QR"])
});
var DigicelMobileMoneyCredentialsSchema = external_exports.object({
  merchantId: external_exports.string(),
  apiKey: external_exports.string(),
  apiSecret: external_exports.string(),
  ussdCode: external_exports.string().optional(),
  paymentBusinessNumber: external_exports.string().optional(),
  integrationType: external_exports.enum(["API", "USSD", "QR"])
});
var KwikPayCredentialsSchema = external_exports.object({
  merchantId: external_exports.string(),
  apiKey: external_exports.string(),
  apiSecret: external_exports.string(),
  integrationType: external_exports.enum(["API", "USSD", "QR"])
});
var LocalEWalletConfigSchema = external_exports.object({
  webhookUrl: external_exports.string().url().optional(),
  signatureVerificationKey: external_exports.string().optional(),
  transactionExpiryMinutes: external_exports.number().int().positive().optional(),
  transactionFeeRate: external_exports.union([external_exports.number(), external_exports.string()]).optional(),
  dailyLimit: external_exports.number().positive().optional(),
  monthlyLimit: external_exports.number().positive().optional(),
  customerPromptText: external_exports.string().optional(),
  logoUrl: external_exports.string().url().optional()
});
var LocalBankConfigSchema = external_exports.object({
  terminalId: external_exports.string().optional(),
  integrationType: external_exports.enum(["HOSTED_REDIRECT", "DIRECT_API_POST"]),
  bankApiEndpointUrl: external_exports.string().url(),
  settlementAccountId: external_exports.string(),
  supportedCurrencies: external_exports.array(external_exports.string()).min(1),
  defaultDisplayCurrency: external_exports.string(),
  merchantDiscountRate: external_exports.union([external_exports.number(), external_exports.string()]).optional(),
  enforce3DSecure: external_exports.boolean(),
  threeDSecureThreshold: external_exports.number().positive().optional(),
  callbackWebhookUrl: external_exports.string().url().optional(),
  checkoutLogoUrl: external_exports.string().url().optional(),
  nameOnCheckout: external_exports.string().optional()
});
var AnzEGateCredentialsSchema = MastercardGatewayCredentialsSchema;
var BredBankCredentialsSchema = external_exports.object({
  merchantId: external_exports.string(),
  accessCode: external_exports.string(),
  secureHashSecret: external_exports.string(),
  apiEndpoint: external_exports.string().url(),
...
var BspBankCredentialsSchema = external_exports.object({
  merchantId: external_exports.string(),
...
var DigitalWalletConfigSchema = external_exports.object({
  paymentProcessorSelector: external_exports.enum(["PAYPAL", "BRED", "BSP"]).optional(),
  // The underlying processor
  applePayMerchantId: external_exports.string().optional(),
  applePayCertificate: external_exports.string().url().optional(),
  // URL or identifier for the certificate
  googlePayMerchantId: external_exports.string().optional()
});
var InternationalFallbackConfigSchema = external_exports.object({
  primaryFallbackGatewaySlug: external_exports.string().optional(),
  // Slug of the chosen international processor
  geoIpTargetingEnabled: external_exports.boolean().optional()
});
var StripeConfigSchema = external_exports.object({
  currency: external_exports.string()
});
var insertPaymentGatewaySchema = createInsertSchema(paymentGateways, {
  priority: external_exports.number().int().default(0),
  // Allow setting priority on insert
  credentials: external_exports.union([
    ...
    // Fallback for truly unknown credentials
  ]).optional(),
...
var selectPublicPaymentSchema = createSelectSchema(payments).pick({
  id: true,
  status: true,
  amount: true,
  currency: true,
...
var insertTourInstanceSchema = createInsertSchema(tourInstances).omit({
  id: true,
  updatedAt: true
});
var insertAvailabilityHoldSchema = createInsertSchema(availabilityHolds).omit({
  id: true,
  createdAt: true
});
var notifications = pgTable("notifications", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id").references(() => users.id),
  // Optional: if null, it's a system/admin notification
...
var PriceResolver = class {
   ...
   */
  calculateItemTotal(adultPax, childPax, rates) {
   ...
   */
  static parseAmountTextToCents(text2) {
    if (!text2) return 0;
    let cleaned = text2.replace(/[VUV$]/g, "").replace(/\/.*/, "").replace(/,/g, "").trim();
...
async function runBackfill() {
  ...
  console.log("\u{1F680} Starting numeric price backfill migration via Neon HTTP driver...");
  const allTours = await db.select().from(tours);
  console.log(`\u{1F4CB} Found ${allTours.length} tours to migrate.`);
  for (const tour of allTours) {
    ...
    await db.update(tours).set({ adultPriceCents, childPriceCents }).where(eq(tours.id, tour.id));
    console.log(`   \u2705 Migrated Tour ${tour.id}: ${tour.price} -> ${adultPriceCents} cents`);
  }
  const allBookings = await db.select().from(bookings);
  console.log(`\u{1F4CB} Found ${allBookings.length} bookings to migrate.`);
  for (const booking of allBookings) {
    const totalAmountCents = PriceResolver.parseAmountTextToCents(booking.amount);
    await db.update(bookings).set({
      totalAmountCents,
      currency: "VUV",
      adultPaxTotal: booking.guests,
      // Assume all guests were adults for legacy data
      childPaxTotal: 0
  ...
  console.log("\u{1F3C1} Migration complete!");
}
