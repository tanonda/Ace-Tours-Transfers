// server/infrastructure/payments/bred-bank.adapter.ts
...
import { BredBankCredentialsSchema, LocalBankConfigSchema } from '../../../shared/schema.js';
import { z } from 'zod';

export type BredBankCredentials = z.infer<typeof BredBankCredentialsSchema>;
export type LocalBankConfig = z.infer<typeof LocalBankConfigSchema>;

export class BredBankAdapter implements PaymentGatewayService {
  private credentials: BredBankCredentials;
  ...
  constructor(gatewayConfig: PaymentGateway) {
    ...
    console.log(`Bred Bank Adapter initialized for ${this.gatewayConfig.slug}`);
  }

  async initiatePayment(request: PaymentInitiationRequest): Promise<PaymentInitiationResponse> {
    console.log(`Bred Bank: Initiating payment for booking ${request.bookingId}`);
    // Simulate API call to Bred Bank
    const transactionId = `BRED-${Date.now()}-${request.bookingId.substring(0, 8)}`;
    ...
    const redirectUrl = `${this.config.bankApiEndpointUrl || this.credentials.apiEndpoint}/initiate?amount=${request.amount}&currency=${request.currency}&returnUrl=${request.successUrl}`;

    return {
      success: true,
      message: 'Payment initiated with Bred Bank.',
  ...
  async handleWebhook(event: WebhookEvent): Promise<WebhookResponse> {
    console.log(`Bred Bank: Handling webhook event for ${event.gatewaySlug}.`);
    // Simulate webhook handling logic
    if (event.signature === 'invalid') {
        return { success: false, message: 'Invalid webhook signature.' };
    }

    const externalTransactionId = event.rawEvent?.transactionId || 'mock-bred-external-id';
  ...
  async queryPaymentStatus(request: PaymentStatusRequest): Promise<PaymentStatusResponse> {
    console.log(`Bred Bank: Querying status for payment ${request.paymentId}`);
    return {
      status: PaymentStatus.Completed,
      gatewayReference: request.gatewayReference || 'mock-bred-ref',
      amount: 1000,
  ...
  async refundPayment(payment: Payment, amount?: number, reason?: string): Promise<PaymentStatusResponse> {
    console.log(`Bred Bank: Refunding payment ${payment.id} for amount ${amount || 'full'}`);
    return {
      status: PaymentStatus.Refunded,
      gatewayReference: payment.gatewayReference || 'mock-bred-refund',
