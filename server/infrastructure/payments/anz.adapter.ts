 ...
 */
export class AnzAdapter implements HostedBankGatewayAdapter {
  readonly providerSlug = 'anz';
  ...
  async initiatePayment(request: PaymentInitiationRequest): Promise<PaymentInitiationResponse & { redirectUrl?: string; gatewayReference?: string }> {
    ...
    console.log(`[ANZ][${mode}] Creating hosted checkout session for booking ${request.bookingId}.`);
    
    return {
      success: true,
      redirectUrl: `https://anz-gateway.example.com/checkout?tx=${Date.now()}`,
      gatewayReference: `anz_ref_${Date.now()}`,
      provider: 'anz'
    };
  }
