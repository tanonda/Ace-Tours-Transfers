...
DELETE FROM payment_gateways WHERE slug IN ('bank-transfer', 'bank_transfer', 'bank');

-- Ensure the canonical bank transfer gateway is active and default
UPDATE payment_gateways SET active = true, is_default = true WHERE slug = 'manual_transfer';
...
VALUES 
  ('payment-bank-transfer', true, 'Bank Transfer', 'Enable manual bank transfer payment method'),
  ('payment-cash-on-delivery', true, 'Cash on Delivery', 'Enable cash on delivery payment method')
