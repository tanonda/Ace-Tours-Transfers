...
import { useLocalizedVehicles } from "@/hooks/useLocalizedProducts";

export default function Vehicles() {
  const { t } = useTranslation();
  const { data: rawVehicles = [], isLoading } = useLocalizedVehicles();

  // Filter out test data and inactive products
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {[...Array(6)].map((_, i) => (
                <div
                  key={i}
                  className="h-[400px] rounded-2xl bg-muted animate-pulse border border-border"
                />
              ))}
            </div>
          ) : vehicles.length === 0 ? (
