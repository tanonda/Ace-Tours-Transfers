...
export default function AdminDashboard() {
  ...
  const filteredBookings = bookings.filter(b => {
    ...
    // Date range filter (only applies when no search query)
    let dateMatch = true;
  ...
  // Use authoritative stats from API (totalRevenueCents includes all confirmed+completed)
  const totalRevenueCents = stats?.totalRevenueCents ?? bookings.reduce((sum, b: any) => {
    if (b.status === 'confirmed' || b.status === 'completed') return sum + (b.totalAmountCents || 0);
    return sum;
  }, 0);
  const pendingCount = stats?.pending ?? bookings.filter((b: any) => b.status === 'pending').length;
  const confirmedCount = stats?.confirmed ?? bookings.filter((b: any) => b.status === 'confirmed').length;
      ...
      <div className="flex flex-col gap-5">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <KPICard
            ...
            value={`VT ${Math.round(totalRevenueCents / 100).toLocaleString()}`}
            delta="+8%"
            icon={DollarSign}
