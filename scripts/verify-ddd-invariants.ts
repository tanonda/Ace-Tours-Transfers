...
async function runTests() {
  ...
  cart.addItem({ productId: "tour1", name: "Tour 1", unitPrice: 100, quantity: 2, date: "2024-01-01" });
  console.log("PASS: Item added to cart");
  
  const snapshot = PricingService.createSnapshot([{ productId: "tour1", name: "Tour 1", price: 100, quantity: 2 }]);
  cart.setPricedSnapshot(snapshot);
  console.log("PASS: Cart priced successfully. Total:", snapshot.total, "VAT:", snapshot.vatAmount);
  ...
  if (Math.abs(booking.amount - snapshot.total) > 0.01) {
    console.error("FAIL: Booking amount mismatch");
  } else {
  ...
  const intent = PaymentIntent.initiate("pay1", booking.id, booking.amount * 100, "Bank Transfer", "anz");
